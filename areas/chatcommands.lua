...
--for mobsplus
--allows players to use /area_mobs to choose what mobs can spawn in an area.
--possible values: "none", "peaceful", "hostile", and "all"
minetest.register_chatcommand("area_mobs", {
	params = "<id> <allow> <chance>",
	description = "Set whether mobs can spawn in specified area, and how often. 'none', 'peaceful', 'hostile', 'all'. chance is in steps, 60 steps = ~1 second.",
	privs = {},
	func = function(name, param)
		local found, _, id, allow = param:find("^(%d+) (.+)$")
		
		if not id then
			minetest.chat_send_player(name, 
				"Invalid usage, see /help area_mobs")
			return
		end
		if not allow then
			minetest.chat_send_player(name, 
		...
		areas.areas[id].mob = allow
		areas.areas[id].chance = chance
		areas:save()
		minetest.chat_send_player(name, "Set mobs that can apprear in area "..id.."to: "..allow)
end})